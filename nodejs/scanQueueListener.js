...
export async function removeSharesJob(dirPaths) {
	const job = await scanQueue.add("removeShares", {
		paths: dirPaths,
	});
	return job;
}
	...
	async (job) => {
		...
		switch (job.name) {
			...
			case "removeUniques":
                ...
                // This should get the existing files from Redis for each size group and add them to the files array
                // filesData.forEach(async ([size, files]) => {
                //     existingFiles = await filesOfSize(size);
                //     console.debug(`Existing files of ${size} : ${existingFiles}`);
                //     console.debug(`New files of ${size} : ${files}`);
                //     files.push(...existingFiles);
                //     console.debug(`Combined files of ${size} : ${files}`);
                // })
			...
			case "removeShares":
				...
				const sharePaths = job.data.paths;
				for (const share of sharePaths) {
					console.debug("Removing share:", share);
					await removePathsStartingWith(share);
				}
				return;
		}
	},
